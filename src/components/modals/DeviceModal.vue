...
const onSaveRecord = handleSubmit(async values => {
  try {
    ...
    if (values?.id) {
      await apolloClient.mutate({
        mutation: UPDATE_ONE_DEVICE,
        fetchPolicy: 'no-cache',
        variables: { id: values.id, input }
      });
    } else {
      await apolloClient.mutate({
        mutation: CREATE_ONE_DEVICE,
        fetchPolicy: 'no-cache',
        variables: { input }
      });
    }
...
<template>
  ...
  >
    ...
    <form v-autocomplete-off class="flex flex-col gap-y-4" @submit.prevent="onSaveRecord">
      <div class="flex flex-col gap-2">
        <label class="font-bold" for="name">Назва пристрою</label>
        <InputText
          id="name"
          v-model="name"
          v-bind="nameAttrs"
      ...
      <div class="flex flex-col gap-2">
        <label class="font-bold" for="description">Опис пристрою</label>
        <Textarea
          id="description"
          v-model="description"
