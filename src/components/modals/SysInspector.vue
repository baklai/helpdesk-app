...
const onSaveReport = () => {
  try {
    ...
    const options = {
      margin: 1,
      filename: `SYSINSPECTOR_${record.value.host} (${new Date(record.value.updatedAt).toLocaleDateString()}).pdf`,
      jsPDF: {
        format: 'a4',
        compress: true,
        floatPrecision: 16,
    ...
    <template #default>
      <div id="report">
        ...
        <div class="my-2 mx-2" v-if="record?.netadapter?.length">
          ...
          <table
            v-for="(netadapter, index) in record?.netadapter?.filter((item) => item?.NetConnectionID) || []"
        ...
        <div class="my-2 mx-2" v-if="record?.videoadapter?.length">
          ...
          <table v-for="(videoadapter, index) in record?.videoadapter || []" :key="`videoadapter_${index}`">
            <tr>
              <td class="font-weight-bold" width="30%">
                {{ $t('Description') }}
              </td>
            ...
            <tr v-for="(user, index) in record?.useraccount || []" :key="`user_${index}`">
              ...
              <td>
                <Chip
                  ...
                  class="border-1"
                  :class="
                    user?.Disabled ? 'text-color-secondary surface-border' : 'text-green-500 border-green-500 font-bold'
                  "
                />
              </td>
            </tr>
        ...
        <div class="my-2 mx-2" v-if="record?.product?.length">
          ...
          <table>
            ...
            <tr v-for="(product, index) in record?.product || []" :key="`product_${index}`">
              <td class="text-center">
                <i class="pi pi-bookmark-fill text-orange-500" v-if="!validSoftware(product?.Name)" />
              </td>
