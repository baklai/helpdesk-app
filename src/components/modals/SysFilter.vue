...
defineExpose({
  toggle: async ({ id }) => {
    try {
      if (id) record.value = await SysFilter.findOne({ id });
      else record.value = SysFilter.$reset();
      records.value = await SysFilter.findAll({});
      records.value = groupBy(records.value, 'type');
      visible.value = true;
    } catch (err) {
      visible.value = false;
      record.value = SysFilter.$reset();
...
const record = ref({});
const records = ref([]);
const $validate = useVuelidate({ regex: { required }, type: { required }, status: { required } }, record);
...
const onRemoveRecord = async () => {
  confirm.require({
    ...
    accept: async () => {
      if (record.value?.id) {
        await SysFilter.removeOne(record.value);
        toast.add({
          severity: 'success',
          summary: t('HD Information'),
...
const onSaveRecord = async () => {
  ...
  if (valid) {
    if (record.value?.id) {
      await SysFilter.updateOne(record.value);
      toast.add({
        severity: 'success',
        summary: t('HD Information'),
      ...
      await SysFilter.createOne(record.value);
      toast.add({
        severity: 'success',
        summary: t('HD Information'),
