...
const { findAll, findOne, createOne, updateOne, removeOne } = useСompany();
...
<template>
  ...
  >
    ...
    <form @submit.prevent="onSaveRecord" class="p-fluid">
      ...
      <div class="field">
        <label>{{ $t('Сompany address') }}</label>
        <InputText v-bind="address" :placeholder="$t('Сompany address')" />
      </div>

      <div class="field">
        <label>{{ $t('Сompany description') }}</label>
        <Textarea
          rows="5"
          class="min-w-full"
          v-bind="description"
