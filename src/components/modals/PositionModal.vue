...
const onSaveRecord = handleSubmit(async values => {
  try {
    ...
    if (values?.id) {
      await apolloClient.mutate({
        mutation: UPDATE_ONE_POSITION,
        fetchPolicy: 'no-cache',
        variables: { id: values.id, input }
      });
    } else {
      await apolloClient.mutate({
        mutation: CREATE_ONE_POSITION,
        fetchPolicy: 'no-cache',
        variables: { input }
      });
    }
...
<template>
  ...
  >
    <template #header>
      <div class="flex w-full justify-between">
        <div class="flex items-center justify-center">
          <i class="pi pi-briefcase mr-4 text-4xl!"></i>
          <div>
            <p class="line-height-2 text-lg font-bold">Посада</p>
    ...
    <form v-autocomplete-off class="flex flex-col gap-y-4" @submit.prevent="onSaveRecord">
      <div class="flex flex-col gap-2">
        <label class="font-bold" for="name">Назва посади</label>
        <InputText
          id="name"
          v-model="name"
          v-bind="nameAttrs"
