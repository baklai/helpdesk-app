...
import { FIND_ONE_MAILBOX, UPDATE_ONE_MAILBOX } from '@/graphql/apollo.gql';

const router = useRouter();
const route = useRoute();
const toast = useToast();

const { result, loading } = useQuery(FIND_ONE_MAILBOX, { id: route.params.id });

const initialValues = computed(() => result.value?.mailbox);

const {
  mutate: update,
  onDone: onDoneUpdate,
  onError: onErrorUpdate
} = useMutation(UPDATE_ONE_MAILBOX, {
