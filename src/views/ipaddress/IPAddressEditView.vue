...
import { FIND_ONE_IPADDRESS, UPDATE_ONE_IPADDRESS } from '@/graphql/apollo.gql';

const router = useRouter();
const route = useRoute();
const toast = useToast();

const { result, loading } = useQuery(FIND_ONE_IPADDRESS, {
  id: route.params.id
});

const initialValues = computed(() => result.value?.ipaddress);

const {
  mutate: update,
  onDone: onDoneUpdate,
  onError: onErrorUpdate
} = useMutation(UPDATE_ONE_IPADDRESS, {
