...
import { FIND_ONE_REQUEST, UPDATE_ONE_REQUEST } from '@/graphql/apollo.gql.js';

const router = useRouter();
const route = useRoute();
const toast = useToast();

const { result, loading } = useQuery(FIND_ONE_REQUEST, { id: route.params.id });
...
const initialValues = computed(() => {
  ...
  return {
    ...req,
    status: req.status ? req.status.toUpperCase() : undefined
  };
});
...
} = useMutation(UPDATE_ONE_REQUEST, {
